...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    init_seeds(1 + RANK)#初始化随机数种子 为了让每次都能复现出一样的结果
    ...
    mlc = int(np.concatenate(dataset.labels, 0)[:, 0].max())  # max label class ?有什么意义
    nb = len(train_loader)  # number of batches 一共128张 batchsize 16 一批16张 所以有8批
    assert mlc < nc, f'Label class {mlc} exceeds nc={nc} in {data}. Possible class labels are 0-{nc - 1}'#用来断言防止出错的
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            model.half().float()  # pre-reduce anchor precision 即将pytorch默认的32位浮点型都改成16位浮点型。
...
def parse_opt(known=False):
    ...
    parser.add_argument('--noval', default=True,action='store_true', help='only validate final epoch')
